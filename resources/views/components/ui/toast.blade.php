...
<div x-data="{ open: true }" x-show="open" x-cloak role="{{ in_array($tone, ['danger', 'error'], true) ? 'alert' : 'status' }}" aria-live="{{ in_array($tone, ['danger', 'error'], true) ? 'assertive' : 'polite' }}" data-tone="{{ $tone }}" {{ $attributes->class(["relative rounded-surface border px-4 py-3 pe-12 text-body-sm leading-6 {$classes}"]) }}>
