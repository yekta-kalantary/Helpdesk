...
<section {{ $attributes->class([
    ...
    @if($title || $subtitle || isset($actions))
        <header class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div class="min-w-0">
                @if($title)<h2 class="break-words font-bold text-workspace-text">{{ $title }}</h2>@endif
                @if($subtitle)<p class="mt-1 break-words text-sm leading-6 text-workspace-muted">{{ $subtitle }}</p>@endif
            </div>
            @isset($actions)
