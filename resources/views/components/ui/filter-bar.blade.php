...
@if($livewire)
    <div {{ $attributes->class(['mb-5 w-full sm:mb-6']) }}>
        <div class="hidden sm:block" data-filter-desktop>
            <div class="mb-3 flex items-center gap-2 text-body-sm font-bold text-text">
                <span>{{ $mobileLabel }}</span>
                <span class="rounded-control bg-surface-muted px-2 py-0.5 text-caption font-semibold text-text-muted" data-active-filter-count>{{ $filterSummary }}</span>
            </div>
        ...
        <details class="mobile-filter-details group sm:hidden" data-filter-mobile>
            <summary class="flex min-h-11 cursor-pointer list-none items-center justify-between rounded-control border border-border bg-surface px-4 text-body-sm font-bold text-text marker:hidden hover:border-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-focus">
