...
<label for="{{ $id }}" {{ $attributes->except(['id'])->class(['flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-3.5 transition hover:border-slate-300']) }}>
    <input id="{{ $id }}" class="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 text-slate-950 focus:ring-slate-400" type="checkbox" name="{{ $name }}" value="{{ $value }}" @checked($checked)>
    <span class="min-w-0">
