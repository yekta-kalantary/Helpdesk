...
<template>
    <div v-if="open" class="fixed inset-0 z-50 lg:hidden" role="presentation">
        <button
            type="button"
            class="absolute inset-0 h-full w-full bg-slate-950/40 motion-reduce:transition-none"
            :aria-label="navigationLabel"
            @click="emit('close')"
        />
        <aside
            id="mobile-navigation"
            class="relative flex h-full w-[min(18rem,calc(100vw-2rem))] max-w-full flex-col bg-white shadow-2xl transition-transform duration-200 ease-out motion-reduce:transition-none"
            role="dialog"
            aria-modal="true"
            :aria-label="navigationLabel"
        >
            <div class="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 px-5">
                <a href="/" class="text-base font-bold tracking-tight text-slate-950">
                    {{ appName }}
                </a>
                <button
                    ref="closeButton"
