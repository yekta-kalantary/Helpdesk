...
@section('content')
    <x-ui.page-header :title="__('tickets::messages.tickets')">
        <x-slot:actions>
            @can('tickets.create')
                <x-ui.button :href="route('tickets.create', ['project' => request('project')])">{{ __('tickets::messages.new_ticket') }}</x-ui.button>
            @endcan
        </x-slot:actions>
    </x-ui.page-header>
    ...
    <x-ui.filter-bar>
        ...
        <div class="min-w-0 flex-1">
            <x-ui.input name="q" :value="request('q')" :placeholder="__('tickets::messages.search_placeholder')" />
        </div>
