...
@section('content')
    <x-ui.page-header :title="__('customers::messages.customers')">
        <x-slot:actions>
            @can('customers.create')
                <x-ui.button :href="route('customers.create')">{{ __('customers::messages.new_customer') }}</x-ui.button>
            @endcan
        </x-slot:actions>
    </x-ui.page-header>
    ...
    <x-ui.filter-bar>
        <div class="min-w-0 flex-1">
            <x-ui.input name="q" :value="request('q')" :placeholder="__('customers::messages.search_placeholder')" />
        </div>
                    ...
                    <td>
                        <div class="flex flex-wrap gap-2">
                            @can('customers.update')
                                <x-ui.button size="sm" variant="secondary" :href="route('customers.edit', $customer['id'])">{{ __('app.edit') }}</x-ui.button>
                            @endcan

                            @can('customers.delete')
                                <form method="POST" action="{{ route('customers.destroy', $customer['id']) }}" onsubmit="return confirm(@js(__('app.confirm_delete')))" >
                                    @csrf
                                    @method('DELETE')
