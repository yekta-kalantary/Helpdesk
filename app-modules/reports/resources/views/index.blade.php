        ...
        <section>
            ...
            <x-ui.table>
                ...
                @forelse($customers as $customer)
                    <tr><td class="font-semibold">{{ $customer->name }}</td><td>{{ $customer->company ?: '—' }}</td><td><x-ui.badge>{{ __('customers::messages.status.'.$customer->status) }}</x-ui.badge></td><td>{{ $customer->projects_count }}</td><td>{{ $customer->open_tickets_count }}</td></tr>
        ...
        <section>
            ...
            <x-ui.table>
                ...
                @forelse($projects as $project)
                    ...
                    <tr><td class="font-semibold">{{ $project->title }}</td><td>{{ $project->customer_name }}</td><td><x-ui.badge>{{ __('projects::messages.status.'.$project->status) }}</x-ui.badge></td><td>{{ $project->tasks_count }}</td><td>{{ $project->done_tasks_count }}</td><td>{{ $project->overdue_tasks_count }}</td><td><x-ui.progress class="w-28" :value="$progress" /></td></tr>
        ...
        <section>
            ...
            <x-ui.table>
                ...
                @forelse($team as $member)
                    <tr><td class="font-semibold">{{ $member->name }}</td><td dir="ltr" class="text-right">{{ $member->email }}</td><td>{{ $member->assigned_tasks_count }}</td><td>{{ $member->done_tasks_count }}</td><td>{{ $member->overdue_tasks_count }}</td></tr>
                @empty<x-ui.empty-row colspan="5" />@endforelse
                </tbody>
            </x-ui.table>
        </section>
