...
@section('content')
    <x-ui.page-header :title="__('app.roles_permissions')">
        @can('roles.create')<x-slot:actions><x-ui.button :href="route('roles.create')">{{ __('identity::messages.new_role') }}</x-ui.button></x-slot:actions>@endcan
    </x-ui.page-header>
        ...
        <x-ui.table>
            ...
            @foreach($roles as $role)
                <tr>
                    ...
                    <td><div class="flex flex-wrap gap-1.5">@foreach($role['permissions'] as $permission)<x-ui.badge dir="ltr">{{ $permission }}</x-ui.badge>@endforeach</div></td>
                    <td>
                        @if(! $role['system'])
                            <div class="flex flex-wrap gap-2">
                                @can('roles.update')<x-ui.button size="sm" variant="secondary" :href="route('roles.edit', $role['id'])">{{ __('app.edit') }}</x-ui.button>@endcan
                                @can('roles.delete')<form method="POST" action="{{ route('roles.destroy', $role['id']) }}" onsubmit="return confirm(@js(__('app.confirm_delete')))" >@csrf @method('DELETE')<x-ui.button size="sm" variant="danger" type="submit">{{ __('app.delete') }}</x-ui.button></form>@endcan
                            </div>
                        @else
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                @foreach(collect($permissions)->groupBy('module') as $module => $modulePermissions)
                    <section class="rounded-xl border border-slate-200 p-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($modulePermissions as $permission)<x-ui.badge dir="ltr">{{ $permission['name'] }}</x-ui.badge>@endforeach
                        </div>
                    </section>
                @endforeach
            </div>
