            ...
            <section ref="dialog" role="dialog" aria-modal="true" aria-labelledby="create-user-title" :aria-describedby="form.hasErrors ? 'create-user-errors' : undefined" class="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
                ...
                <form class="grid gap-5 p-5" @submit.prevent="submit">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <label class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.last_name }}<Input v-model="form.last_name" name="last_name" autocomplete="family-name" :aria-invalid="!!fieldError('last_name')" :aria-describedby="fieldError('last_name') ? 'last-name-error' : undefined" /> <span v-if="fieldError('last_name')" id="last-name-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('last_name') }}</span></label>
                    </div>
                    <label class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.email }}<Input v-model="form.email" type="email" name="email" autocomplete="email" :aria-invalid="!!fieldError('email')" :aria-describedby="fieldError('email') ? 'email-error' : undefined" /> <span v-if="fieldError('email')" id="email-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('email') }}</span></label>
                    <label class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.mobile }}<Input v-model="form.mobile" name="mobile" autocomplete="tel" :aria-invalid="!!fieldError('mobile')" :aria-describedby="fieldError('mobile') ? 'mobile-error' : undefined" /> <span v-if="fieldError('mobile')" id="mobile-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('mobile') }}</span></label>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.role }}<select v-model="form.role" name="role" class="h-9 rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" :aria-invalid="!!fieldError('role')" :aria-describedby="fieldError('role') ? 'role-error' : undefined" @change="onRoleChange"><option value="" disabled>{{ copy.form.role_placeholder }}</option><option v-for="role in roles" :key="role" :value="role">{{ copy.roles[role] || role }}</option></select><span v-if="fieldError('role')" id="role-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('role') }}</span></label>
                        <label v-if="form.role === 'customer'" class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.client }}<select v-model="form.client_id" name="client_id" class="h-9 rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" :aria-invalid="!!fieldError('client_id')" :aria-describedby="fieldError('client_id') ? 'client-error' : undefined"><option :value="null" disabled>{{ copy.form.client_placeholder }}</option><option v-for="client in clients" :key="client.id" :value="client.id">{{ client.name }}</option></select><span v-if="fieldError('client_id')" id="client-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('client_id') }}</span></label>
                    </div>
                    <label class="flex items-center gap-2 text-sm font-medium text-slate-700"><input v-model="form.is_active" type="checkbox" name="is_active" :disabled="form.password_mode === 'email'" class="size-4 rounded border-input text-primary focus:ring-primary" :aria-invalid="!!fieldError('is_active')" :aria-describedby="fieldError('is_active') ? 'is-active-error' : undefined" />{{ copy.form.active }}<span v-if="fieldError('is_active')" id="is-active-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('is_active') }}</span></label>
                    <fieldset class="grid gap-3" :aria-invalid="!!fieldError('password_mode')" :aria-describedby="fieldError('password_mode') ? 'password-mode-error' : undefined"><legend class="text-sm font-medium text-slate-700">{{ copy.form.password_mode }}</legend><label class="flex items-start gap-2 text-sm"><input v-model="form.password_mode" type="radio" value="manual" name="password_mode" :aria-invalid="!!fieldError('password_mode')" :aria-describedby="fieldError('password_mode') ? 'password-mode-error' : undefined" @change="onPasswordModeChange" /><span>{{ copy.form.manual_password }}<small class="block text-muted-foreground">{{ copy.form.manual_hint }}</small></span></label><label class="flex items-start gap-2 text-sm"><input v-model="form.password_mode" type="radio" value="email" name="password_mode" :aria-invalid="!!fieldError('password_mode')" :aria-describedby="fieldError('password_mode') ? 'password-mode-error' : undefined" @change="onPasswordModeChange" /><span>{{ copy.form.email_password }}<small class="block text-muted-foreground">{{ copy.form.email_hint }}</small></span></label><span v-if="fieldError('password_mode')" id="password-mode-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('password_mode') }}</span></fieldset>
                    <div v-if="form.password_mode === 'manual'" class="grid gap-4 sm:grid-cols-2"><label class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.password }}<Input v-model="form.password" type="password" name="password" autocomplete="new-password" :aria-invalid="!!fieldError('password')" :aria-describedby="fieldError('password') ? 'password-error' : undefined" /> <span v-if="fieldError('password')" id="password-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('password') }}</span></label><label class="grid gap-1.5 text-sm font-medium text-slate-700">{{ copy.form.password_confirmation }}<Input v-model="form.password_confirmation" type="password" name="password_confirmation" autocomplete="new-password" :aria-invalid="!!fieldError('password_confirmation')" :aria-describedby="fieldError('password_confirmation') ? 'password-confirmation-error' : undefined" /> <span v-if="fieldError('password_confirmation')" id="password-confirmation-error" role="alert" class="text-xs font-normal text-red-700">{{ fieldError('password_confirmation') }}</span></label></div>
